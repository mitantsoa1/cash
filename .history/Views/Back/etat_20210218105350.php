        ...
        <!-- <div class="row"> -->
                    ...
                    <?php 
						...
						$tab = $db ->selectSP()
									->from("adresse")
									->executeSP();
						$select= '<select name="select_adresse_etat" id="select_adresse_etat" class="form-control adresse_etat">';
									for($i=0;$i<count($tab);$i++){
                                        $select.='<option value="'.$tab[$i]->lot.'">'.$tab[$i]->lot.'-'.$tab[$i]->emplacement.'</option>';
										// $select .= '<option value="'.$tab[$i]->emplacement.'">'.$tab[$i]->numero.'</option>';
									}
									$select .= '<option value="tous" selected="selected">Tous</option></select>';
									echo $select;
					?>
    ...
    <div>
        <?php 
                    ...
                    $table = $db->selectSP()
								...
								->executeSP();
						echo " <br><table class='table table-striped table-hover table-bordered table_solde_etat hide' id='myTable'>
								<thead>
									<th>Numero</th>
                                    <th>Solde</th>
                                    <th>LOT</th>
     ...
     <div class="etat_recherche container hide">
       
       </div>
       <button id="exportEtat" class="btn btn-success hide" onclick="exportTableToExcel('tableEtat')">Export to excel</button>
