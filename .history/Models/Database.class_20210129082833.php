<?php
    class Database {
        ...
        public function outfile($chemin,$term,$enclose,$lines){
            $this->query .= " INTO OUTFILE '".$chemin."' FIELDS TERMINATED BY '".$term."' OPTIONALLY ENCLOSED BY '".$enclose."' LINES TERMINATED BY '".$lines."'";
            return $this;
        }
}
        // public function selectCom($tab){
        
        //     $query="SELECT * FROM commission where ";
    
        //     for($i=0;$i<count($tab);$i++){
                
        //         $query.= "operateur LIKE %".$tab[$i]."% or montant_min LIKE %".$tab[$i]."% or montant_max LIKE %".$tab[$i]."% or commission_d LIKE %".$tab[$i]."% or commission_r LIKE %".$tab[$i]."% or commission_t LIKE %".$tab[$i]."%";
