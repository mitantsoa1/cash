...
if (isset($_POST['recherche'])) {
    ...
    $tb = $db->selectSP()
        ->from("commission")
        ->where("operateur", "=")
        ->et(["montant_min", "montant_max", "commission_d", "commission_r", "commission_t"], ["LIKE", "LIKE", "LIKE", "LIKE", "LIKE"])
        ->execute([$_POST['operateur'], "%" . $recherche . "%", "%" . $recherche . "%", "%" . $recherche . "%", "%" . $recherche . "%", "%" . $recherche . "%"]);
    echo json_encode($tb);
}
