		...
		header('Content-type: application/json');
				...
				// }else if(isset($_POST['telma'])){
                    ...
                    if (isset($_POST['tableau'])){
                         $tab = htmlspecialchars(trim($_POST['tableau']));

                    }
                    
                    // insertion solde

                    if(isset($_POST['operateur_c'])){
                         ...
                         $t = $db->selectSP()
                                   ->from('solder')
                                   ->where('operateur',"=")
                                   ->execute([$_POST['operateur_c']]);

                                   $sold = $t[0]->solde + $depot;

                         $db ->insert("depot")
                              ->parametters(['operateur','montant','description'])
                              ->execute([$_POST['operateur_c'],$depot,$observation]);

                         $db->update("solder")
                              ->set(['solde','observation'])
                              ->where("operateur","=")
                              ->execute([$sold,$observation,$_POST['operateur_c']]);
                              echo json_encode("$sold");

                              // $sum_depot = $db->sum('solder','solde')
                              //                          ->where('operateur','=')
                              //                          ->execute([$_POST['operateur']]);
                    }
