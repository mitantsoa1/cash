                    ...
                    if(isset($_POST['number_trans'])){
                         ...
                         if($_POST['type']=='retrait'){
                              $db ->insert("transaction")
                                   ...
                                   ->execute([$operateur,$num_tr,$_POST['type'],$montant,$numero,$observation,$frais_r,$commission_r,$_POST['lieu']]);
                         }else if($_POST['type']=='depot'){
                              $db ->insert("transaction")
                                   ->parametters(['operateur','number','type','montant','numero','observation','frais','lieu'])
                                   ->execute([$operateur,$num_tr,$_POST['type'],$montant,$numero,$observation,$frais_d,$commission_d,$_POST['lieu']]);
                         }else if($_POST['type']=='transfert'){
                              $db ->insert("transaction")
                                   ->parametters(['operateur','number','type','montant','numero','observation','frais','lieu'])
                                   ->execute([$operateur,$num_tr,$_POST['type'],$montant,$numero,$observation,$frais_t,$commission_t,$_POST['lieu']]);
                         }
